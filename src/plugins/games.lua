...
plugin.commands.test = function (args)
    ...
    args.modules.irc.privmsg(args.target, ('Testing %s: [\x03%s\x03]'):format(message, res))
end
...
plugin.commands.coinflip = function (args)
    ...
    args.modules.irc.privmsg(args.target, ('heads: %d tails: %d'):format(heads, tails))
end
...
plugin.commands.magic8ball = function (args)
    ...
    if message:len() % 3 == 0 then
        reply = predictions.positive[math.random(10)]
    elseif message:len() % 3 == 1 then
...
plugin.commands.is = function (args)
    ...
    args.modules.irc.privmsg(args.target, ('%s %s%s'):format(prob[r1], case[r2], punct[r3]))
end
...
plugin.help = ('usage: games <%s>'):format(h:sub(2))
