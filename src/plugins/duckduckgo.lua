...
plugin.main = function(args)
    ...
    if not search then
        modules.irc.privmsg(args.target, 'You want me to search for what?')
        return
    end
    ...
    if resp then
        ...
        modules.irc.privmsg(args.target, search:find('^!') and bang(resp) or plain(resp))
        return
    end

    modules.irc.privmsg(args.target, 'something went wrong with the request')
end
