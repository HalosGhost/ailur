...
plugin.commands.get = function (args)
    ...
    args.modules.irc.privmsg(args.connection, args.target, tostring(args.modules.config[setting]))
end

plugin.commands.type = function (args)
    local _, _, setting = args.message:find('type%s+(%S+)')
    args.modules.irc.privmsg(args.connection, args.target, type(args.modules.config[setting]))
end
