...
plugin.commands.kick = function (args)
    ...
    modules.irc.kick(args.target, recipient, message or recipient)
end
...
plugin.commands['set-mode'] = function (args)
    ...
    modules.irc.mode(args.target, recipient, mode)
end
...
plugin.commands.join = function (args)
    ...
    if channel then
        modules.irc.join(channel)
        modules.irc.privmsg(args.target, ('Joined %s'):format(channel))
    end
end
...
plugin.commands.hatroulette = function (args)
    ...
    modules.irc.privmsg(args.target, ('%s rolls for a %s!'):format(args.sender, mode))
    modules.irc[mode == 'kick' and 'kick' or 'modeset'](args.target, args.sender, mode)
end
